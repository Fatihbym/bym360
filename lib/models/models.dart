...
class GetBelgeGetir {
  ...
  factory GetBelgeGetir.fromJson(Map<String, dynamic> json) {
    return GetBelgeGetir(
      durum: (json['DURUM'] ?? 0) as int,
      fisTur: (json['FISTUR'] ?? 0) as int,
      fisTarih: json['FISTARIH'] ?? '',
      depoAdi: json['DEPOADI'] ?? '',
      cariAdi: json['CARIADI'] ?? '',
      subeAdi: json['SUBEADI'] ?? '',
      oPlanAdi: json['OPLANADI'] ?? '',
      fisId: (json['FISID'] ?? 0) as int,
      belgeNo: json['BELGENO'] ?? '',
      depoId: (json['DEPOID'] ?? 0) as int,
      cariId: (json['CARIID'] ?? 0) as int,
      oPlanId: (json['OPLANID'] ?? 0) as int,
      subeId: (json['SUBEID'] ?? 0) as int,
      aciklama: json['ACIKLAMA'] ?? '',
      tutar: (json['TUTAR'] ?? 0).toDouble(),
      ad: json['AD'] ?? '',
      barkod: json['BARKOD'] ?? '',
      kod: json['KOD'] ?? '',
      miktar: (json['MIKTAR'] ?? 0).toDouble(),
      mevcutMiktar: (json['MEVCUTMIKTAR'] ?? 0).toDouble(),
      fisTurAdi: json['FISTURADI'] ?? '',
      onay: (json['ONAY'] ?? 0) as int,
      toplamMiktar: (json['TOPLAM_MIKTAR'] ?? 0).toDouble(),
      stokGrup1: json['StokGrup1'] ?? '',
    );
  }
}
